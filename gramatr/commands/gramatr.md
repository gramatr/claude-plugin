...
Note inline that full grāmatr-block drift detection in `CLAUDE.md` is
deferred to #3128 Phase 4 — this verb checks the pin, version, and session,
not the injected block.
...
- Phase 3 adds the per-platform system-prompt contract and
  `get_system_prompt` tool. The v1 verb wave (#3288) makes `whoami`,
  `status`, `update`, and `init` real; Wave 2 (#3288) adds `handoff show`,
  `project list`, `project fix`, `reflect`, `feedback`, and `recall`, and
  hardens `handoff` with the committed-pin-vs-resolved mismatch guard. Full
  grāmatr-block drift detection in `CLAUDE.md` remains Phase 4 of #3128.
