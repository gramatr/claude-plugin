...
argument. If no argument is provided, behaves like `/gramatr:gramatr help`.

## Invocation form (read this first)

This command file ships under the `gramatr` plugin, so Claude Code namespaces it
as `/gramatr:gramatr <verb>`. The bare `/gramatr <verb>` form does NOT invoke
once the plugin is installed — always teach and print the namespaced
`/gramatr:gramatr <verb>` form in any user-facing output (help table, error
hints, drift hints). The verb-parse examples below show the bare form only to
illustrate tokenization AFTER dispatch — they are not the form the user types.
...
Everything after the verb is forwarded as the sub-argument. (The leading
`/gramatr:gramatr` is the namespaced command name; the verb is the first
argument token after it.)

Examples (post-dispatch tokenization — the user types the `/gramatr:gramatr`
prefix shown in the help table):

- `/gramatr:gramatr` → verb is empty → show help
- `/gramatr:gramatr help` → verb is `help` → show help
...
grāmatr status
  ...
  project     <slug> (<project_id>)        — or "not pinned (run /gramatr:gramatr project)"
  ...
  statusline  installed   — or "not installed (run /gramatr:gramatr statusline)"
  block       current   — or "n/a" — or "DRIFTED: run /gramatr:gramatr-update"
```

If drift is flagged, add one line: suggest `/gramatr:gramatr project` to re-pin, and
...
concern from the plugin binary), point the user at `/gramatr:gramatr-update`.
...
If none yield a UUID, tell the user to run `/gramatr:gramatr project` first
...
- **`where_we_are`** — current branch, latest commit, server/test status,
  what just happened in this session. **First-sentence contract:** the FIRST
  SENTENCE of this section becomes the user-visible checkpoint summary on the
  statusline (first sentence only, ≤80 chars, derived verbatim). Lead with the
  single most important current fact (e.g. "v0.24.2 released; #3555 coverage
  fix is the open thread") — NEVER a timestamp, vibe, or label like
  "~07:31Z, deep multi-hour session".
...
> under a pre-rehome `project_id` — run `/gramatr:gramatr project fix` to reunite it.
...
#### PROJECT LIST (`/gramatr:gramatr project list`)
...
`/gramatr:gramatr project list` can be re-run to page further. Do not mutate anything.
...
#### PROJECT FIX (`/gramatr:gramatr project fix [target]`)
...
- Phase 2 of #3128 migrated `/handoff`, `/gramatr-setup-project`, and
  ...
  verb (legacy-only artifact). The block-refresh command remains separate from
  this verb shell; under the installed plugin it invokes as
  `/gramatr:gramatr-update` (the bare `/gramatr-update` does not invoke), and it
  refreshes the bundled grāmatr block in `CLAUDE.md`.
- Invocation namespacing (#3347): every command in this plugin is reached via
  the `/gramatr:<command>` form — the dispatcher is `/gramatr:gramatr <verb>`,
  and the native wave-1 verbs are `/gramatr:whoami`, `/gramatr:recall`,
  `/gramatr:feedback`, `/gramatr:reflect`. Bare `/gramatr <verb>` does NOT
  invoke once the plugin is installed; user-facing help/error text must always
  print the namespaced form.
