...
function getTurnBufferPath(projectDir) {
  return (0, import_node_path5.join)(projectDir, GRAMATR_DIR2, TURNS_FILE);
}
...
function writeBufferedTurns(projectDir, turns) {
  ...
  const dest = getTurnBufferPath(projectDir);
  const tmp = `${dest}.tmp.${process.pid}`;
  (0, import_node_fs5.writeFileSync)(tmp, JSON.stringify(turns, null, 2) + "\n", "utf8");
...
function clearBufferedTurns(projectDir) {
  try {
    (0, import_node_fs5.rmSync)(getTurnBufferPath(projectDir), { force: true });
  } catch {
  }
}
...
function parseTranscript(transcriptPath) {
  try {
    const transcriptContent = (0, import_node_fs6.readFileSync)(transcriptPath, "utf8");
...
function findProjectRoot(startDir = process.cwd()) {
  let dir = startDir;
  for (; ; ) {
    if ((0, import_node_fs7.existsSync)((0, import_node_path6.join)(dir, GRAMATR_DIR3)))
      return dir;
...
function canonicalizeProjectRoot(dir) {
  ...
  if ((0, import_node_fs7.existsSync)((0, import_node_path6.join)(mainRoot, GRAMATR_DIR3)))
    return mainRoot;
  return dir;
}
...
function snapshotPath(projectDir) {
  return (0, import_node_path8.join)(projectDir, GRAMATR_DIR4, SNAPSHOT_FILE);
}
function readSnapshot(projectDir) {
  try {
    ...
    if (!(0, import_node_fs9.existsSync)(dest))
      return null;
    const parsed = JSON.parse((0, import_node_fs9.readFileSync)(dest, "utf8"));
...
function writeSnapshot(projectDir, snap) {
  try {
    const dir = (0, import_node_path8.join)(projectDir, GRAMATR_DIR4);
    if (!(0, import_node_fs9.existsSync)(dir))
      (0, import_node_fs9.mkdirSync)(dir, { recursive: true, mode: 448 });
    const dest = snapshotPath(projectDir);
    const tmp = `${dest}.tmp.${process.pid}`;
    (0, import_node_fs9.writeFileSync)(tmp, JSON.stringify(snap) + "\n", "utf8");
...
function parsePorcelain(stdout) {
  ...
  for (const rawLine of stdout.split("\n")) {
    ...
    if (!path)
      continue;
    if (path === GRAMATR_DIR4 || path.startsWith(`${GRAMATR_DIR4}/`))
      continue;
...
// dist/hooks/lib/turn-capture.js
...
async function flushBufferedTurns(projectDir, fetchImpl = fetch) {
  ...
  const result = await chunkFlushTurns(turns, url, token, {
