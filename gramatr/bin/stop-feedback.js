...
function findProjectRoot(startDir = process.cwd()) {
  let dir = startDir;
  for (; ; ) {
    if ((0, import_node_fs4.existsSync)((0, import_node_path3.join)(dir, GRAMATR_DIR3)))
      return dir;
...
function canonicalizeProjectRoot(dir) {
  ...
  if ((0, import_node_fs4.existsSync)((0, import_node_path3.join)(mainRoot, GRAMATR_DIR3)))
    return mainRoot;
  return dir;
}
...
var defaultGitRunner = (args, cwd) => {
  try {
    const res = (0, import_node_child_process2.spawnSync)("git", args, {
      cwd,
...
function snapshotPath(projectDir) {
  return (0, import_node_path5.join)(projectDir, GRAMATR_DIR4, SNAPSHOT_FILE);
}
function readSnapshot(projectDir) {
  try {
    ...
    if (!(0, import_node_fs6.existsSync)(dest))
      return null;
    const parsed = JSON.parse((0, import_node_fs6.readFileSync)(dest, "utf8"));
...
function writeSnapshot(projectDir, snap) {
  try {
    const dir = (0, import_node_path5.join)(projectDir, GRAMATR_DIR4);
    if (!(0, import_node_fs6.existsSync)(dir))
      (0, import_node_fs6.mkdirSync)(dir, { recursive: true, mode: 448 });
    const dest = snapshotPath(projectDir);
    const tmp = `${dest}.tmp.${process.pid}`;
    (0, import_node_fs6.writeFileSync)(tmp, JSON.stringify(snap) + "\n", "utf8");
