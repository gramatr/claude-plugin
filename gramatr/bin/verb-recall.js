...
function attachDpopProof(keyPair, method, url) {
  if (!keyPair)
    return {};
  try {
    return { DPoP: buildDpopProof(keyPair, { htm: method, htu: url }) };
  } catch {
    return {};
  }
}
function getProcessDpopKeyPair() {
  if (!processKeyPair) {
    processKeyPair = generateDpopKeyPair();
  }
  return processKeyPair;
}
var import_node_crypto2, DPOP_JWT_TYP, processKeyPair;
var init_dpop_key = __esm({
  "dist/hooks/lib/dpop-key.js"() {
    "use strict";
    init_dist();
    import_node_crypto2 = require("node:crypto");
    DPOP_JWT_TYP = "dpop+jwt";
    processKeyPair = null;
  }
});
...
function buildDeviceProof(keyPair, params) {
  return buildCompactEnvelope(keyPair.privateKey, keyPair.publicJwk, DEVICE_JWT_TYP, {
    nonce: params.nonce,
    iat: params.iat ?? Math.floor(Date.now() / 1e3),
    jti: params.jti ?? b64url((0, import_node_crypto3.randomBytes)(16)),
    ...params.backend ? { backend: params.backend } : {}
  });
}
var import_node_crypto3, DEVICE_JWT_TYP;
...
function reconstructDeviceKeyPair(privateJwk) {
  const privateKey = (0, import_node_crypto4.createPrivateKey)({ key: privateJwk, format: "jwk" });
...
var import_node_child_process3, import_node_fs3, import_node_path3, import_node_crypto4, spawnImpl2, platformImpl2, KEYRING_SERVICE2, KEYRING_ACCOUNT2, FILE_BACKEND_NAME2, KEYRING_CMD_TIMEOUT_MS2;
...
function dpopHeader(method, url, keyPair = getProcessDpopKeyPair()) {
...
// dist/bin/verb-recall.js
var verb_recall_exports = {};
__export(verb_recall_exports, {
  runVerbRecall: () => runVerbRecall
});
module.exports = __toCommonJS(verb_recall_exports);
...
async function verbFetch(token, method, path, body, fetchImpl = fetch) {
  ...
  try {
    const res = await fetchImpl(url, {
      method,
      headers: {
        // Bearer (#3374) + RFC 9449 DPoP proof (#3382) — the proof sender-
        // constrains the bearer to this process's ephemeral in-memory key.
        // Additive: a server not yet validating DPoP ignores the extra header.
        ...sessionAuthHeaders(token, method, url),
...
// dist/bin/verb-recall.js
var DEFAULT_LIMIT = 8;
async function runVerbRecall(argv = process.argv.slice(2)) {
  const query = argv.join(" ").trim();
  if (!query) {
    emit("gr\u0101matr recall: provide a query, e.g. `/recall handoff hardening`.");
    return;
  }
  ...
  const res = await verbFetch(token, "POST", "search", { query, limit: DEFAULT_LIMIT });
  if (!res.ok) {
    emit(`gr\u0101matr recall: search failed (${res.error}).`);
  ...
  if (results.length === 0) {
    emit(`gr\u0101matr recall: no matches for "${query}" in this project's knowledge graph.`);
    return;
  }
  const lines = [`gr\u0101matr recall \u2014 top ${results.length} for "${query}"`, ""];
  results.forEach((r, i) => {
    const type = r.entity_type ?? r.type ?? "entity";
    const score = typeof r.similarity_score === "number" ? r.similarity_score.toFixed(3) : "\u2014";
    lines.push(`${i + 1}. [${type}] ${r.name ?? "(unnamed)"}  (score ${score})`);
    if (r.snippet)
      lines.push(`   ${r.snippet.replace(/\s+/g, " ").trim()}`);
  });
  emit(lines.join("\n"));
}
if (process.argv[1] && /verb-recall(\.js|\.ts)?$/.test(process.argv[1])) {
  runVerbRecall().catch(() => {
    emit("gr\u0101matr recall: unexpected error running search.");
  });
}
