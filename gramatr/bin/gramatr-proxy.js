...
var import_node_fs2, import_node_path2, GRAMATR_DIR2, SESSION_FILE, SESSION_TOKEN_EXPIRY_SKEW_MS;
...
var import_node_child_process2, import_node_fs3, import_node_path3, spawnImpl, platformImpl, KEYRING_SERVICE, KEYRING_ACCOUNT, FILE_BACKEND_NAME, KEYRING_CMD_TIMEOUT_MS;
...
var import_node_child_process3, import_node_fs5, import_node_path5, import_node_crypto3, spawnImpl2, platformImpl2, KEYRING_SERVICE2, KEYRING_ACCOUNT2, FILE_BACKEND_NAME2, KEYRING_CMD_TIMEOUT_MS2;
...
var MINT_LOCK_FILE = "token-mint.lock";
var MINT_LOCK_STALE_MS = 5e3;
async function withMintLock(dataDir, fn) {
  try {
    if (!(0, import_node_fs6.existsSync)(dataDir))
      (0, import_node_fs6.mkdirSync)(dataDir, { recursive: true });
    const lockPath = (0, import_node_path6.join)(dataDir, MINT_LOCK_FILE);
    if (!(0, import_node_fs6.existsSync)(lockPath))
      (0, import_node_fs6.writeFileSync)(lockPath, "", { mode: 384 });
...
async function mintProxyTokenFromKeyring(dataDir, baseUrl, fetchImpl = fetch, now = Date.now(), knownStaleToken) {
  if (!isMintCredentialUsable(readMintCredential(), now)) {
    return { status: "no_credential" };
  }
  return withMintLock(dataDir, async () => {
    const current = readProxyToken(dataDir);
    if (current && knownStaleToken && current.token !== knownStaleToken) {
      return { status: "minted", record: current };
    }
...
async function reMintProxyBearer(knownStaleToken) {
  if (!PLUGIN_DATA_DIR)
    return false;
  const outcome = await mintProxyTokenFromKeyring(PLUGIN_DATA_DIR, REMOTE_BASE, fetch, Date.now(), knownStaleToken);
...
async function maybeProactivelyMint() {
  ...
  await reMintProxyBearer(record?.token);
}
async function forwardWithLadder(message) {
  ...
  if (first.httpStatus !== 401)
    return first;
  const staleToken = PLUGIN_DATA_DIR ? readProxyToken(PLUGIN_DATA_DIR)?.token : void 0;
  const reMinted = await reMintProxyBearer(staleToken);
  if (!reMinted) {
    return first;
  }
