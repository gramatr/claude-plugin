...
function recordTurn1Arming(args) {
  ...
  try {
    ...
    const jsonlPath = (0, import_node_path4.join)(dir, "turn1-arming.jsonl");
...
function recordBootstrapRecovered(args) {
  ...
  try {
    ...
    if (!(0, import_node_fs4.existsSync)(dir))
      (0, import_node_fs4.mkdirSync)(dir, { recursive: true });
    const jsonlPath = (0, import_node_path4.join)(dir, "bootstrap-recovered.jsonl");
    const line = JSON.stringify(sample);
    let existing = [];
    if ((0, import_node_fs4.existsSync)(jsonlPath)) {
      existing = (0, import_node_fs4.readFileSync)(jsonlPath, "utf8").split("\n").filter((l) => l.trim().length > 0);
...
function appendHookPromotionSample(sample) {
  try {
    ...
    if (!(0, import_node_fs4.existsSync)(dir))
      (0, import_node_fs4.mkdirSync)(dir, { recursive: true });
    const jsonlPath = (0, import_node_path4.join)(dir, "hook-promotion.jsonl");
    const lastPath = (0, import_node_path4.join)(dir, "last-sample.json");
    const line = JSON.stringify(sample);
    let existing = [];
    if ((0, import_node_fs4.existsSync)(jsonlPath)) {
      existing = (0, import_node_fs4.readFileSync)(jsonlPath, "utf8").split("\n").filter((l) => l.trim().length > 0);
    ...
    (0, import_node_fs4.writeFileSync)(jsonlPath, existing.join("\n") + "\n", "utf8");
    (0, import_node_fs4.writeFileSync)(lastPath, JSON.stringify(sample, null, 2), "utf8");
  } catch {
  }
}
var import_node_fs4, import_node_path4, MAX_SAMPLES;
...
function getServerVersionPath(projectDir) {
  return (0, import_node_path3.join)(projectDir, GRAMATR_DIR2, SERVER_VERSION_FILE);
}
...
function readLastSeenServerVersion(projectDir) {
  try {
    ...
    if (!(0, import_node_fs3.existsSync)(path))
      return null;
    const raw = (0, import_node_fs3.readFileSync)(path, "utf8").trim();
...
function findProjectRoot(startDir = process.cwd()) {
  let dir = startDir;
  for (; ; ) {
    if ((0, import_node_fs5.existsSync)((0, import_node_path5.join)(dir, GRAMATR_DIR3)))
      return dir;
...
function canonicalizeProjectRoot(dir) {
  ...
  if ((0, import_node_fs5.existsSync)((0, import_node_path5.join)(mainRoot, GRAMATR_DIR3)))
    return mainRoot;
  return dir;
}
...
async function computeUpsRoute(input, fetchImpl = fetch, projectDirOverride) {
  ...
  const bearerExpiresAt = resolveClientBearerExpiry(resolveMcpUrl());
