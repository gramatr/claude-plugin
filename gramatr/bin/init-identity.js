...
function writeSessionToken(projectDir, file) {
  ...
  (0, import_node_fs6.writeFileSync)(tmp, JSON.stringify(file, null, 2) + "\n", { encoding: "utf8", mode: 384 });
...
async function fetchBootstrapPayload(token, clientSessionId) {
  ...
  try {
    ...
    const restToken = rt && typeof rt.token === "string" && rt.token && typeof rt.expires_at === "string" && typeof rt.base_url === "string" && typeof rt.aud === "string" ? { token: rt.token, expires_at: rt.expires_at, base_url: rt.base_url, aud: rt.aud } : void 0;
