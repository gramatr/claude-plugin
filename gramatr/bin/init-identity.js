...
function writeSessionJson(payload, clientSessionId) {
  ...
  const dir = (0, import_node_path6.join)(PROJECT_DIR, ".gramatr");
  const target = (0, import_node_path6.join)(dir, "session.json");
  try {
    const prev = JSON.parse((0, import_node_fs6.readFileSync)(target, "utf8"));
  ...
  try {
    ...
    const tmp = (0, import_node_path6.join)(dir, `session.json.tmp.${process.pid}`);
    (0, import_node_fs6.writeFileSync)(tmp, JSON.stringify(next, null, 2) + "\n", "utf8");
    (0, import_node_fs6.renameSync)(tmp, target);
  } catch {
  }
  try {
