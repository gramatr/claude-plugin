...
function legacyActiveSession(projectDir) {
  const legacySession = readJson((0, import_node_path3.join)(projectDir, GRAMATR_DIR, "session.json"));
...
function fileRead() {
  try {
    const dest = getFileBackendPath2();
    if (!(0, import_node_fs10.existsSync)(dest))
      return null;
    const raw = (0, import_node_fs10.readFileSync)(dest, "utf8").trim();
...
function getFileBackendPath2() {
  return (0, import_node_path10.join)(getHomeDir(), ".gramatr", FILE_BACKEND_NAME2);
}
...
function getToken2() {
  ...
  if (PLUGIN_DATA_DIR) {
    try {
      const cfg = JSON.parse((0, import_node_fs11.readFileSync)((0, import_node_path11.join)(PLUGIN_DATA_DIR, "token.json"), "utf8"));
  ...
  try {
    const credFile = (0, import_node_path11.resolve)(HOME_DIR, ".claude", ".credentials.json");
...
function writeSessionJson(payload, clientSessionId, model) {
  ...
  const dir = (0, import_node_path11.join)(PROJECT_DIR, ".gramatr");
  const target = (0, import_node_path11.join)(dir, "session.json");
  let sessionJsonUnchanged = false;
  try {
    const prev = JSON.parse((0, import_node_fs11.readFileSync)(target, "utf8"));
  ...
  if (!sessionJsonUnchanged) {
    try {
      ...
      const tmp = (0, import_node_path11.join)(dir, `session.json.tmp.${process.pid}`);
      (0, import_node_fs11.writeFileSync)(tmp, JSON.stringify(next, null, 2) + "\n", "utf8");
      (0, import_node_fs11.renameSync)(tmp, target);
    } catch {
    }
  }
  try {
...
async function main() {
  ...
  try {
    (0, import_node_fs11.unlinkSync)((0, import_node_path11.join)(PROJECT_DIR, ".gramatr", "ctx-tokens.json"));
  } catch {
  }
