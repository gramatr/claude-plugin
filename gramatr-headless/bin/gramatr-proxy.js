...
function resolveProxyVersion() {
  ...
  try {
    const here = (0, import_node_path5.dirname)((0, import_node_url.fileURLToPath)(import_meta.url));
    for (const candidate of [
      (0, import_node_path5.join)(here, "..", "..", "package.json"),
      (0, import_node_path5.join)(here, "..", "package.json"),
      (0, import_node_path5.join)(here, "..", "..", "..", "package.json")
    ]) {
...
function getToken() {
  ...
  if (PLUGIN_DATA_DIR) {
    try {
      const cfg = JSON.parse((0, import_node_fs5.readFileSync)((0, import_node_path5.join)(PLUGIN_DATA_DIR, "token.json"), "utf8"));
  ...
  try {
    const credFile = (0, import_node_path5.resolve)(HOME_DIR, ".claude", ".credentials.json");
...
`);
  ...
  if (PLUGIN_DATA_DIR) {
    (0, import_node_fs5.mkdirSync)(PLUGIN_DATA_DIR, { recursive: true });
    (0, import_node_fs5.writeFileSync)((0, import_node_path5.join)(PLUGIN_DATA_DIR, "token.json"), JSON.stringify({ token: accessToken }, null, 2) + "\n", "utf8");
  }
...
function buildBootstrapNoOpResult(projectDir) {
  try {
    const raw = (0, import_node_fs5.readFileSync)((0, import_node_path5.join)(projectDir, ".gramatr", "session.json"), "utf8");
...
function getProjectId(cwd) {
  try {
    const projectFile = (0, import_node_path5.join)(cwd, ".gramatr", "project.json");
    if ((0, import_node_fs5.existsSync)(projectFile)) {
      const data = JSON.parse((0, import_node_fs5.readFileSync)(projectFile, "utf8"));
...
async function handleMessage(msg) {
  ...
  if (method === "tools/call" && msg.params !== null && typeof msg.params === "object") {
    ...
    if (params.name === "session_bootstrap") {
      ...
      const sessionJsonExists = (0, import_node_fs5.existsSync)((0, import_node_path5.join)(bootstrapProjectDir, ".gramatr", "session.json"));
