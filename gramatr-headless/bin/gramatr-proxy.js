...
var spawnImpl = import_node_child_process.spawnSync;
...
function resolveProxyVersion() {
  ...
  try {
    ...
    for (const candidate of [
      (0, import_node_path4.join)(here, "..", "..", "package.json"),
      (0, import_node_path4.join)(here, "..", "package.json"),
      (0, import_node_path4.join)(here, "..", "..", "..", "package.json")
    ]) {
      if ((0, import_node_fs4.existsSync)(candidate)) {
        const pkg = JSON.parse((0, import_node_fs4.readFileSync)(candidate, "utf8"));
...
function getToken() {
  ...
  if (PLUGIN_DATA_DIR) {
    try {
      const cfg = JSON.parse((0, import_node_fs4.readFileSync)((0, import_node_path4.join)(PLUGIN_DATA_DIR, "token.json"), "utf8"));
  ...
  try {
    const credFile = (0, import_node_path4.resolve)(HOME_DIR, ".claude", ".credentials.json");
...
`);
  ...
  if (PLUGIN_DATA_DIR) {
    (0, import_node_fs4.mkdirSync)(PLUGIN_DATA_DIR, { recursive: true });
    (0, import_node_fs4.writeFileSync)((0, import_node_path4.join)(PLUGIN_DATA_DIR, "token.json"), JSON.stringify({ token: accessToken }, null, 2) + "\n", "utf8");
  }
...
function buildBootstrapNoOpResult(projectDir) {
  try {
    const raw = (0, import_node_fs4.readFileSync)((0, import_node_path4.join)(projectDir, ".gramatr", "session.json"), "utf8");
...
function getGitRemote(cwd) {
  try {
    const result = (0, import_node_child_process2.spawnSync)("git", ["-C", cwd, "remote", "get-url", "origin"], {
      timeout: 2e3,
      encoding: "utf8"
    });
...
function getProjectId(cwd) {
  try {
    const projectFile = (0, import_node_path4.join)(cwd, ".gramatr", "project.json");
    if ((0, import_node_fs4.existsSync)(projectFile)) {
      const data = JSON.parse((0, import_node_fs4.readFileSync)(projectFile, "utf8"));
...
async function handleMessage(msg) {
  ...
  if (method === "tools/call" && msg.params !== null && typeof msg.params === "object") {
    ...
    if (params.name === "session_bootstrap") {
      ...
      const sessionJsonExists = (0, import_node_fs4.existsSync)((0, import_node_path4.join)(bootstrapProjectDir, ".gramatr", "session.json"));
