...
- **Server reports a newer client version** → tell the user to update. The
  server stamps an `update_available` signal (installed vs latest) which surfaces
  as a `⚠ update → vX.Y.Z` segment on the statusline; if you see it (or an
  `update_available` block in a tool response), relay the channel-specific
  upgrade step — for the marketplace plugin: **update via `/plugin` (grāmatr)
  then `/reload-plugins`**.
